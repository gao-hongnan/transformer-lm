...
# if hasattr(torch.optim.optimizer, "ParamsT"):  # higher torch version
#     ParamsT: TypeAlias = torch.optim.optimizer.ParamsT  # type: ignore[name-defined]
# else:
#     ParamsT: TypeAlias = torch.optim.optimizer.params_t  # type: ignore[misc, no-redef]
...
class AdamW(torch.optim.Optimizer):
    ...
    def validate_params(
        ...
        if not 0.0 <= weight_decay:
            raise ValueError(
                f"Invalid weight_decay value: {weight_decay}, must be >= 0."
            )
