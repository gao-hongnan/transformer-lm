    ...
    def generate(
        ...
        *,
        ...
        if isinstance(starting_tokens, list):
            starting_tokens = cast(
                torch.LongTensor,
                torch.as_tensor(starting_tokens, dtype=torch.long)[None, ...],
            )
        ...
        for _ in range(max_tokens):
            ...
            seq_len = starting_tokens_cropped.size(
                1
            )  # this must be less than or equal to self.config.context_length
            ...
            next_token = (
                ...
                if not greedy
                else torch.topk(probs, k=1, dim=-1).indices
            )
